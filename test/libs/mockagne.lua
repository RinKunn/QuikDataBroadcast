    ...
    for i = 1, #self.expected_returns do
        ...
        if (candidate.mock == self and candidate.key == method and compareArgs({...}, candidate.args)) then
            if candidate.tp == 'val' then 
				...
				return candidate.returnFn(unpack(candidate.returnArgs))
			end
        end
    end
    return nil
...
--todo
local function expectFn(self, method, returnFn, returnArgs, ...)
    local expectation = { mock = self, key = method, returnFn = returnFn, returnArgs = returnArgs, args = {...}, tp = 'fn' }
    table.insert(self.expected_returns, expectation)
end
--todo
local function thenAnswerFn(fn, ...)
    latest_invoke.mock:expectFn(latest_invoke.key, fn, {...}, unpack(latest_invoke.args))
...
function M.when(fcall)
    return { thenAnswer = thenAnswer,thenAnswerFn = thenAnswerFn }
end
